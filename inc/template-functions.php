...
if ( ! function_exists( 'ezpzconsultations_body_classes' ) ) {
  /**
   ...
   * @return array
   */
  function ezpzconsultations_body_classes( $classes ) {
...
add_filter( 'body_class', 'ezpzconsultations_body_classes' );

if ( ! function_exists( 'ezpzconsultations_pingback_header' ) ) {
  /**
   * Add a pingback url auto-discovery header for single posts, pages, or attachments.
   */
  function ezpzconsultations_pingback_header() {
    if ( is_singular() && pings_open() ) {
      printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
    }
  }
}
add_action( 'wp_head', 'ezpzconsultations_pingback_header' );

if ( ! function_exists( 'ezpzconsultations_yoastprioritylow' ) ) {
  /**
   * Move Yoast Meta Box to bottom of editor
   */
  function ezpzconsultations_yoastprioritylow() {
    return 'low';
  }
}
add_filter( 'wpseo_metabox_prio', 'ezpzconsultations_yoastprioritylow');

if ( ! function_exists( 'ezpzconsultations_unset_image_sizes' ) ) {
  /**
  ...
  function ezpzconsultations_unset_image_sizes($sizes) {
...
add_action('intermediate_image_sizes_advanced', 'ezpzconsultations_unset_image_sizes');
 ...
 * @return void
 */
if ( !function_exists( 'ezpzconsultations_change_search_url' )) :
  function ezpzconsultations_change_search_url() {
    if ( is_search() && ! empty( $_GET['s'] ) ) {
      wp_redirect( home_url( "/search/" ) . urlencode( get_query_var( 's' ) ) ); // Redirect to slug search
      exit();
    }
  }
  add_action( 'template_redirect', 'ezpzconsultations_change_search_url' );
endif;
 ...
 */
add_filter( 'admin_post_thumbnail_html', 'ezpzconsultations_featured_image_admin_prompt', 10, 3 );
if ( !function_exists( 'ezpzconsultations_featured_image_admin_prompt' )) :
  function ezpzconsultations_featured_image_admin_prompt( $content, $post_id, $thumbnail_id ){
    $help_text = '<p>' . __( 'Please add a featured image. This will be used as the main image for the page on search engines and may be displayed on the page itself depending on the design. ', 'ezpzconsultations' ) . '</p>';
 ...
 *
 */
if ( ! function_exists( 'ezpzconsultations_get_post_primary_category' ) ) :
  function ezpzconsultations_get_post_primary_category($post_id, $term='category', $return_all_categories=false){
    $return = array();
 ...
 */
add_filter( 'walker_nav_menu_start_el', 'ezpzconsultations_add_navbar_descriptions', 10, 4 );
if ( ! function_exists( 'ezpzconsultations_add_navbar_descriptions' ) ) :
  function ezpzconsultations_add_navbar_descriptions( $item_output, $item, $depth, $args ) {
...
* Yoast SEO hooks into this function to populate the %%excerpt%% tag with ezpzconsultations_filter_wpseo_excerpt()
*
...
*/
if (! function_exists('ezpzconsultations_generate_excerpt') ) :
  function ezpzconsultations_generate_excerpt($trim = null, $ellipses = false) {
    ...
    foreach ($possible_excerpts as $possible_excerpt) {
      if($possible_excerpt === 'sections') {
        $post_excerpt = ezpzconsultations_excerpt_from_acf_flexible_content();
      }
      else {
...
/**
 * Replaces Excerpt Variable in Yoast SEO by using function ezpzconsultations_generate_excerpt()
 ...
 * @return void
 */
add_filter( 'wpseo_replacements', 'ezpzconsultations_filter_wpseo_excerpt', 10, 1 );
if (! function_exists('ezpzconsultations_filter_wpseo_excerpt') ) :
  function ezpzconsultations_filter_wpseo_excerpt($replacements) {
    global $post;
      $replacements['%%excerpt%%'] = ezpzconsultations_generate_excerpt(160, true);
    return $replacements;
  }
endif;
