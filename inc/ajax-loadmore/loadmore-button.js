...
jQuery(function($){ // use jQuery code inside this to avoid "$ is not defined" error
	$('.button-loadmore').click(function(){
		...
		var button = $(this),
		    data = {
        'action': 'loadmore',
        'query': ezpzconsultations_loadmore_params.posts, // that's how we get params from wp_localize_script() function
      ...
      scrollTo = ajaxContainer.offset().top + ajaxContainer.height(); // Bottom of the container, so that we can scroll back to it when new content loaded
		...
		$.ajax({ // you can also use $.post here
			url : ezpzconsultations_loadmore_params.ajaxurl, // AJAX handler
			data : data,
			type : 'POST',
